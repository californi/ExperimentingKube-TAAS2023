...
strategy activateNoFailureRate [ NoFailureRate ] {  
  t0: (NoFailureRate) -> addHighScalabilityHighQuality() @[65000 /*ms*/] {
    t0a: (success) -> done;
  }   
  t1: (default) -> TNULL;
}
...
strategy activateLowFailureRate [ LowFailureRate && replicasOver ] {  
  t0: (LowFailureRate) -> activateLowScalabilityHighQuality() @[65000 /*ms*/] {
    t0a: (success) -> done;
  }      
  t1: (default) -> TNULL;
}
...
strategy activateHighFailureRate [ HighFailureRate ] {    
  t0: (HighFailureRate && replicasOver) -> addLowScalabilityLowQuality() @[65000 /*ms*/] {
    t0a: (success) -> done;
  }   
  t1: (default) -> TNULL;
}
...
strategy ImproveSlo [ sloRed ] {
  ...
  t1: (sloRed && !canAddReplica) -> lowerFidelity() @[45000 /*ms*/] {
    t1a: (success) -> done;
  }
  t2: (default) -> TNULL;
}
...
strategy ReduceCostImproveFidelity [ sloGreen ] {
  t0: (canRemoveReplica) -> removeReplica() @[45000 /*ms*/] {
    t0a: (success) -> done;
  }
  t1: (textMode || lowMode) -> raiseFidelity() @[45000 /*ms*/] {
    t1a: (success) -> done;
  }  
  t2: (default) -> TNULL;
}