...
strategy deactivateScalabilityB [ NoFailureRate && (!canAddReplica || IsScalabilityb) ] {  
  t0: (NoFailureRate && (!canAddReplica || IsScalabilityb)) -> removeLowScalability() @[65000 /*ms*/] {
    t0a: (success) -> done;
  }   
  t1: (default) -> TNULL;
}
...
strategy activateScalabilityB [ LowFailureRate || HighFailureRate ] {  
  t0: (!IsScalabilityb && (LowFailureRate || HighFailureRate)) -> addLowScalability() @[65000 /*ms*/] {
    t0a: (success) -> done;
  }      
  t1: (default) -> TNULL;
}